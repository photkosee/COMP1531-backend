...
describe('Testing success unpinning message - message/unpin/v1', () => {
  test('Channel messages', () => {
    ...
    const unpin = JSON.parse(res.getBody() as string);
    ...
    const unpin2 = JSON.parse(res.getBody() as string);
    expect(res.statusCode).toBe(OK);
    expect(unpin2).toStrictEqual({});
  });
  ...
  test('Dm messages', () => {
    ...
    const unpin3 = JSON.parse(res.getBody() as string);
    expect(res.statusCode).toBe(OK);
    expect(unpin3).toStrictEqual({});
    ...
    const unpin4 = JSON.parse(res.getBody() as string);
    expect(res.statusCode).toBe(OK);
    expect(unpin4).toStrictEqual({});
  });
});
